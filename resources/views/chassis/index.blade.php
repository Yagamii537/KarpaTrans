...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div>
        <h4 class="fw-semibold mb-1">Chasis</h4>
        ...
        <form method="GET"
            ...
            <div class="col-md-5">
                <input type="text"
                       name="search"
                       class="form-control"
                       placeholder="Buscar código, placa o número de serie"
                       value="{{ $search }}">
            </div>
            ...
            <div class="col-md-3">
                <select name="status" class="form-select">
                    ...
                    <option value="OUT_OF_SERVICE" @selected($status === 'OUT_OF_SERVICE')>Fuera de servicio</option>
                </select>
            </div>
            ...
            <table class="table align-middle">
                ...
                <tbody>
                    @forelse ($chassisList as $chassis)
                        <tr>
                            <td>
                                <div class="fw-semibold">{{ $chassis->code }}</div>
                            ...
                            <td>
                                {{ $chassis->vehicle?->display_name
                                    ?: 'Sin asignación habitual' }}
                            </td>

                            <td>
                                <div>{{ $chassis->chassis_type }}</div>
                                <small class="text-muted">
                                    {{ $chassis->axles ?: '-' }} ejes
                                    · {{ $chassis->maximum_capacity_tons ?: '-' }} t
                                </small>
                            </td>
                            ...
                            <td>
                                ...
                                @if ($chassis->supports_40ft)
                                    <span class="badge bg-primary-subtle text-primary">40'</span>
                                @endif

                                @if ($chassis->supports_reefer)
                                    <span class="badge bg-info-subtle text-info">Reefer</span>
                                @endif
                            </td>
                            ...
                            <td class="text-end">
                                ...
                                <a href="{{ route('chassis.edit', $chassis) }}"
                                   class="btn btn-sm btn-outline-primary">
                                    <i class="ti ti-edit"></i>
                                </a>

                                <form method="POST"
                                      action="{{ route('chassis.destroy', $chassis) }}"
                                      class="d-inline"
                                      onsubmit="return confirm('¿Eliminar este chasis?')">
