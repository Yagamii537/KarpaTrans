<div class="row">
    ...
    <div class="col-md-4 mb-3">
        <label class="form-label">Vehículo habitual</label>
            ...
            @foreach ($vehicles as $availableVehicle)
                <option value="{{ $availableVehicle->id }}"
                    @selected(
                        old(
                            'vehicle_id',
                            $chassis->vehicle_id ?? request('vehicle_id')
                        ) == $availableVehicle->id
                    )>
                    {{ $availableVehicle->display_name }}
                </option>
            @endforeach
        </select>
    </div>
    ...
    <div class="col-md-3 mb-3">
        ...
        <select name="chassis_type" class="form-select" required>
            @foreach([
                ...
                <option value="{{ $value }}"
                    @selected(old('chassis_type', $chassis->chassis_type ?? 'PORTACONTENEDOR') === $value)>
                    {{ $label }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-md-4 mb-3">
        ...
        <select name="operational_status" class="form-select">
            @foreach([
                ...
                <option value="{{ $value }}"
                    @selected(old('operational_status', $chassis->operational_status ?? 'AVAILABLE') === $value)>
                    {{ $label }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-12 mb-3">
        ...
        <div class="d-flex flex-wrap gap-4">
            ...
            <div class="form-check">
                <input type="checkbox"
                       name="supports_reefer"
                       value="1"
                       class="form-check-input"
                       id="supports_reefer"
                       @checked(old('supports_reefer', $chassis->supports_reefer ?? false))>
    ...
    <div class="col-md-6 mb-3">
        ...
        <input type="date"
               ...
               value="{{ old(
                   ...
                   isset($chassis?->registration_expiration_date)
                       ? $chassis->registration_expiration_date->format('Y-m-d')
                       : ''
               ) }}">
    </div>
    ...
    <div class="col-md-4 mb-3">
        ...
        <input type="file"
               name="technical_review_document"
               accept=".pdf,.jpg,.jpeg,.png"
               class="form-control">
    </div>
    ...
    <div class="col-12 mb-3">
        ...
        <textarea name="notes"
                  rows="3"
                  class="form-control">{{ old('notes', $chassis->notes ?? '') }}</textarea>
    </div>
